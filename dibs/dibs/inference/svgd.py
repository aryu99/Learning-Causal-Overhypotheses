...
class JointDiBS(DiBS):
    ...
    def sample(self, *, key, n_particles, steps, n_dim_particles=None, callback=None, callback_every=None, return_z=False):
        """
        ...
        Arguments:
            ...
            callback: function to be called every ``callback_every`` steps of SVGD.
            callback_every: if ``None``, ``callback`` is only called after particle updates have finished
            return_z (bool): if ``True``, return latent particles :math:`Z` in addition to :math:`(G, \\Theta)`
        ...
        if return_z:
            return g_final, theta_final, z_final
...
class VQDiBS(DiBS):
    """Vector-Quantized DiBS. Experiments at compostionality of latent space.
    ...
    def __init__(self, *,
        ...
        self.num_codebook_per_node = num_codebook_per_node
        self.num_nodes = num_nodes
        self.particle_dim = particle_dim


        # Initialize codebook of latent Z values
        codebook = jax.random.ball(key, particle_dim, p=1, shape=(num_codebook_per_node, ))
        self.codebook = codebook.reshape((particle_dim, num_codebook_per_node))
    ...
    def _sample_initial_random_particles(self, *, key, n_particles, n_dim=None):
        ...
        # z = random.normal(subk, shape=(n_particles, self.n_vars, n_dim, 2)) * std
        weights = jax.nn.softmax(random.uniform(subk, shape=(n_particles, 2, self.num_nodes, self.num_codebook_per_node)))
        z = jnp.einsum("jk, xyik -> xjiy", self.codebook, weights)
